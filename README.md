...
- `backend/` - a Java 21 Gradle project (Spring Boot: core/persistence/api) containing the library logic you will
  extend.
...
- Backend helpers (JDK 21):
    ...
    - Perf smoke (short, local): `node tools/run-perf.mjs` (API must be running; set `PERF_BASE_URL`/`PERF_ITERATIONS`
      to override)
...
The backend is a Spring Boot application (modules: core/persistence/api). Domain rules are intentionally naive; your job
is to fix them while keeping the API surface unchanged.
...
1) **Prevent double loans and respect queues**
    ...
    - If a reservation queue exists, only the head of the queue should receive the book (no line-jumping via direct
      borrow).
...
2) **Reservation lifecycle**
    ...
    - Returning a book must hand it to the next eligible reserver in order; skip ineligible/missing members and
