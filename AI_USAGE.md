...
- Succeeded to connect DB to IDE to be able to see what's in the tables. To see tables need to adjust absolute path in Database IntelliJ settings.
...
- Almost all tasks were quite easy for me. Excepting a few bugs in initial project version ;). Still it was very interesting and exciting task - I have never done
  correction of the code before. I spent much time alone completing `kood/Jõhvi` Java FullStack specialization during my study time there - some of them quite complicated. So it
...
- `Gradle` tests passed.
- Unfortunately erased first version (no history of my work is available) submitted on the 17th of December, due to mistake when have uploaded corrected version of the task.
