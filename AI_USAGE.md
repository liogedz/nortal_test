...
```bash
cd keys
# Generates private key (PKCS#8, unencrypted)
...
- Created controller to get
  JWT [DevTokenController](backend/api/src/main/java/com/nortal/library/api/controller/DevTokenController.java)
- Run `GET http://localhost:8080/dev` (added it to permitAll() in `SecurityConfig`) request to get JWT token
  in [requests.http](requests.http) for checking endpoints (or any other tool like `Postman`- need to create script to
  copy JWT to variable for further usage or copy it manually from response)
- `requests.http` save token to variable, may check some requests.
- Almost all tasks were quite easy for me. Excepting a few bugs in initial project version ;). Still it was very
  interesting and exciting task - I have never done correction of the code before. I spent much time alone completing
  `kood/Jõhvi` Java FullStack specialization during my study time there - some of them quite complicated. So it gave me
  good background completing this one.
